...
interface PaneViewProps {
  ...
  getTerminalClearTrigger?: (terminalInstanceId: string) => number | undefined; // Made optional
...
const PaneView: React.FC<PaneViewProps> = React.memo(({
  pane, terminalInstances, onTerminalData, onTerminalResize, getTerminalDataToDisplay, getTerminalClearTrigger, isActivePane, onPaneClick,
}) => {
      ...
      <Allotment vertical={pane.direction === 'vertical'} key={pane.id}>
        {pane.children.map(childPane => (
          <Allotment.Pane key={childPane.id} preferredSize={childPane.size || '100%'}>
            <PaneView {...{ pane: childPane, terminalInstances, onTerminalData, onTerminalResize, getTerminalDataToDisplay, getTerminalClearTrigger, isActivePane, onPaneClick }} />
          </Allotment.Pane>
        ))}
      </Allotment>
    );
